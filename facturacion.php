...
if (
    isset($_POST['Nombre_Factura']) && strlen($_POST['Nombre_Factura']) >= 1 &&
    isset($_POST['TotalPagar_Factura']) && strlen($_POST['TotalPagar_Factura']) >= 1 &&
    isset($_POST['MetodoPago_Factura']) && strlen($_POST['Metodo_Factura']) >= 1 &&
    isset($_POST['Correo_Factura']) && strlen($_POST['Correo_Factura']) >= 1 &&
    isset($_POST['Productos_Factura']) && strlen($_POST['Productos_Factura']) >= 1 &&
    isset($_POST['Fecha_Factura']) && strlen($_POST['Fecha_Factura']) >= 1
) {
    $nombre = trim($_POST['Nombre_Factura']);
    $totalPagar = trim($_POST['TotalPagar_Factura']);
    $metodoPago = trim($_POST['MetodoPago_Factura']);
    $correo = trim($_POST['Correo_Factura']);
    $productos = trim($_POST['Productos_Factura']);
    $fecha = trim($_POST['Fecha_Factura']);

    $consulta = "INSERT INTO facturas(Nombre_Factura, TotalPagar_Factura, Metodo_Factura, Correo_Factura, Productos_Factura, Fecha_Factura) VALUES ('$nombre', '$totalPagar', '$metodoPago', '$correo', '$productos', '$fecha')";
...
<head>
  ...
  <style>
    ...
    .registro {
      ...
      padding: 20px;
      margin: 170px auto 20px; /* Añadir un margen superior para separar del nav */
      width: 450px;
      height: 760px;
      border-radius: 56px;
    ...
    <section class="registro">
      ...
      <form action="facturacion.php" method="post">
        ...
        <input
          class="controls"
          type="text"
          name="TotalPagar_Factura"
          id="TotalPagar_Factura"
          placeholder="Total a Pagar"
        />
        <?php echo $mensaje; ?>
        <input class="botons" type="submit" value="Facturar" />
      </form>
    </section>
